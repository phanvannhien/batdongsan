...
function wp_pagenavi( $args = array() ) {
	...
	switch ( intval( $options['style'] ) ) {
		...
		case 1:
			...
			if ( !empty( $options['pages_text'] ) ) {
				...
				$options['pages_text'] );
				$out .= "<li><a class='pages'>$pages_text</a></li>";
			}
			...
			foreach ( range( $start_page, $end_page ) as $i ) {
				if ( $i == $paged && !empty( $options['current_text'] ) ) {
					...
					$out .= "<li class='active'><a class='current'>$current_page_text</a></li>";
					$timeline = 'larger';
				} else {
	...
	$out = $before . "<div class='wp-pagenavi pagination'><ul>\n$out\n</ul></div>" . $after;
